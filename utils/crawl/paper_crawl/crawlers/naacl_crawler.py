...
def naacl_crawler():
    """
    기존 함수 - conference_list.json에서 NAACL 관련 컨퍼런스들을 모두 크롤링
    """
    ...
    for field in config['fields']:
        for conf in field['conferences']:
            if conf.get('crawler') == 'naacl_crawler':
    ...
    # 전체 최종 저장
    output_path = os.path.join(base_dir, "naacl_papers_2024.json")
