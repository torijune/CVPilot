...
def fetch_paper_titles_and_links(url: str):
    ...
    # CVPR 사이트에 맞는 선택자: class="ptitle"인 dt 태그 안의 a 태그
...
def cvpr_crawler():
    """
    기존 함수 - conference_list.json에서 CVPR 관련 컨퍼런스들을 모두 크롤링
    """
    ...
    for field in config['fields']:
        for conf in field['conferences']:
            if conf.get('crawler') == 'cvpr_crawler':
    ...
    # 전체 최종 저장
    output_path = os.path.join(base_dir, "cvpr_papers_2024.json")
